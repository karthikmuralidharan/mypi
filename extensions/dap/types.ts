...
/**
 * Adapter-initiated event (`stopped`, `output`, `terminated`, `initialized`, ...).
 *
 * LOCAL FIX: upstream @piex-dev/dap v0.1.1 references this type in the DapMessage
 * union and imports it in client.ts, but never defines it — the published package
 * does not typecheck. Shape follows the DAP spec and the fields actually consumed
 * by client.ts (`msg.event`, `msg.body`) and session.ts.
 */
...
export type DapInitializedEventBody = {};
...
export interface DapAdapterConfig {
	...
	/**
	 * stdio  — DAP framing over the adapter's stdin/stdout (debugpy, gdb, lldb-dap).
	 * socket — adapter picks a port and announces it on stdout; we scrape and dial (dlv).
	 * tcp    — we reserve a free port and pass it via `${port}` in args, then dial (js-debug).
	 */
